...
var formatSeconds = function (value) {
    var secondTime = parseInt(String(value)); // 秒
    var minuteTime = 0; // 分
    var hourTime = 0; // 小时
    if (secondTime > 60) {
    ...
    var result = '' + (parseInt(String(secondTime)) < 10 ? '0' + parseInt(String(secondTime)) : parseInt(String(secondTime)));
    result =
        '' +
 ...
 */
var contrastTime = function (timeInfo) {
    if (!timeInfo) {
        return false;
    }
    var timeInfoGet = String(timeInfo).replace(/-/g, '/');
...
var formatTime = function (timeInfo) {
    ...
    var timeInfoGet = String(timeInfo).replace(/-/g, '/');
    var date = new Date(Number(timeInfoGet));
 ...
 * date，给定时间
 * **/
var dateFormatSet = function (fmt, date) {
    var ret;
    var opt = {
